...
CommonMarkGFM::Iterator - Iterate CommonMarkGFM nodes
...
C<CommonMarkGFM::Iterator> provides a convenient way to walk through the nodes
in a parse tree.
...
Event types are:
    ...
    CommonMarkGFM::EVENT_DONE
    CommonMarkGFM::EVENT_ENTER
    CommonMarkGFM::EVENT_EXIT

Event types can be imported from L<CommonMarkGFM> with tag C<event>.

    use CommonMarkGFM qw(:event);
...
Enter the node. The event type is C<CommonMarkGFM::EVENT_ENTER> and the current
...
C<CommonMarkGFM::EVENT_EXIT> and the current node is set to the original node
C<V>.

=back

After the root node was exited, the event type is set to
C<CommonMarkGFM::EVENT_DONE> and the current node to C<undef>. In scalar
context, C<next> returns C<CommonMarkGFM::EVENT_DONE>. In list context, it
returns the empty list.

For leaf nodes, no exit events are generated. Leaf nodes comprise the node
types that never have children:

    CommonMarkGFM::NODE_HTML
    CommonMarkGFM::NODE_HRULE
    ...
    CommonMarkGFM::NODE_INLINE_HTML
