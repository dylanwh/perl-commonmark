...
BEGIN {
    use_ok('CommonMarkGFM', 'OPT_DEFAULT');
}
...
is(CommonMarkGFM->markdown_to_html($md), $expected_html, 'markdown_to_html');
...
my $rendered_md = $doc->render_CommonMarkGFM(OPT_DEFAULT, 20);
my $expected_md = <<'EOF';
# Header
...
**strong**
EOF
is($rendered_md, $expected_md, 'render_CommonMarkGFM');

is(CommonMarkGFM->markdown_to_html("\x{263A}"), "<p>\x{263A}</p>\n",
   'render functions return encoded utf8');

is(CommonMarkGFM->markdown_to_html("\xC2\xA9"), "<p>\xC2\xA9</p>\n",
